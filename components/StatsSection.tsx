...
const StatsSection: React.FC = () => {
  ...
  const stats = [
    {
      number: '40+',
      label: lang === 'ar' ? 'مدرسة عريقة تخدم آلاف الطلاب' : 'established schools serving thousands',
      color: 'bg-rose-400',
      shape: 'rounded-[30%_70%_70%_30%/30%_30%_70%_70%]' // Organic blob shape
    },
    {
      number: '50k+',
      label: lang === 'ar' ? 'طالب نشط في منظومتنا' : 'active students in our network',
      color: 'bg-sky-300',
      shape: 'rounded-full'
    },
    {
      number: '8.5k+',
      label: lang === 'ar' ? 'معلم متميز وخبير تربوي' : 'expert educators and specialists',
      color: 'bg-amber-300',
      shape: 'rounded-[60%_40%_30%_70%/60%_30%_70%_40%]'
    },
    {
      number: '5',
      label: lang === 'ar' ? 'محافظات مصرية نغطيها' : 'governorates across Egypt',
      color: 'bg-emerald-300',
      shape: 'rounded-2xl rotate-12'
    }
  ];

  const dateStr = lang === 'ar' ? 'رحلتنا في أرقام:' : 'Our Journey in Numbers:';
