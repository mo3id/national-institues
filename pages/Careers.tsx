    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-blue-900 py-24 text-white text-center">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <p className="text-base md:text-xl text-blue-200">{t.subtitle}</p>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-4 -mt-6 mb-24 pt-5 md:pt-8 lg:pt-12">
        <div className={`grid grid-cols-1 lg:grid-cols-3 gap-12`}>
          {/* Job Listings */}
          <div className="lg:col-span-2 space-y-6">
            <h2 className="text-2xl md:text-3xl font-bold mb-6 text-gray-900 text-start">{t.openings}</h2>
            {JOBS.map((job) => (
              <div
                key={job.id}
              ...
              >
                ...
                <div className={`flex ${isRTL ? 'space-x-reverse space-x-6' : 'space-x-6'} text-sm md:text-base text-gray-500 mb-6`}>
                ...
                <p className="text-gray-600 line-clamp-2 md:line-clamp-3 mb-6 text-sm md:text-base">{job.description}</p>
                <button className={`text-blue-600 font-bold text-sm md:text-base flex items-center ${isRTL ? 'space-x-reverse space-x-2' : 'space-x-2'}`}>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-xl sticky top-24 border border-gray-100 text-start">
              {submitted ? (
                <div className="text-center py-12 space-y-4">
                  <div className="flex justify-center">
                    <CheckCircle className="h-16 w-16 text-green-500" />
                  </div>
                  <h3 className="text-xl md:text-2xl font-bold">{t.successTitle}</h3>
                  <p className="text-sm md:text-base text-gray-500">{t.successDesc}</p>
                  <button onClick={() => setSubmitted(false)} className="text-blue-600 font-bold underline">{t.applyAnother}</button>
                </div>
              ) : (
                <form onSubmit={handleApply} className="space-y-6">
                  <h3 className="text-lg md:text-xl font-bold">{t.applyNow}</h3>
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-sm md:text-base font-bold text-gray-700">{t.fullName}</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-sm md:text-base font-bold text-gray-700">{t.email}</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-sm md:text-base font-bold text-gray-700">{t.jobRef}</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-sm md:text-base font-bold text-gray-700">{t.upload}</label>
                  ...
                  <button type="submit" className="w-full bg-blue-600 text-white py-3 md:py-4 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition-all transform hover:scale-[1.02]">
                    {t.submit}
                  </button>
                </form>
              )}
