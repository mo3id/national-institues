...
const LoginPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            if (email === 'admin@nis.edu.eg' && password === 'admin123') {
                ...
                setError('Invalid email or password. Use admin@nis.edu.eg / admin123');
            }
            setLoading(false);
        }, 1500);
    };
    ...
    return (
        <PageTransition>
            <div className="min-h-screen flex items-center justify-center bg-[#f8faff] p-4 font-inter relative overflow-hidden">
                    ...
                    <div className="bg-white rounded-[2rem] p-10 md:p-12 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-white relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <div className="flex flex-col items-center mb-10 text-center">
                                ...
                                <p className="text-slate-400 mt-2 font-bold text-xs uppercase tracking-widest leading-relaxed">Enter credentials to access dashboard</p>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group/input">
                                        <Mail className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-300 group-focus-within/input:text-blue-600 transition-colors" />
                                        <input
                                            type="email"
                                            placeholder="admin@nis.edu.eg"
                                            className="w-full h-14 pl-14 pr-6 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-600 focus:bg-white transition-all font-bold text-slate-700 placeholder:text-slate-300"
                                            value={email}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group/input">
                                        <Lock className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-300 group-focus-within/input:text-blue-600 transition-colors" />
                                        <input
                                            type={showPassword ? 'text' : 'password'}
                                            placeholder="••••••••"
                                            className="w-full h-14 pl-14 pr-14 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-600 focus:bg-white transition-all font-bold text-slate-700 placeholder:text-slate-300"
                                            value={password}
                            ...
                            <p className="mt-12 text-center text-slate-300 text-[10px] font-black uppercase tracking-[0.3em] italic">
                                National Institutes Schools Network
                            </p>
                        </div>
                    </div>
                </div>
