  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      <div className="flex flex-col items-center mb-12">
        <div className="bg-blue-100 text-blue-700 px-4 py-1 rounded-full text-sm font-bold flex items-center space-x-2 mb-4">
          <Sparkles className="h-4 w-4" />
          <span>Powered by Gemini 3.1 & Veo</span>
        </div>
        <h1 className="text-4xl md:text-5xl font-bold text-center">NIS AI Studio</h1>
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
            <div className="space-y-6">
              {/* Contextual Settings */}
              {activeTool === 'generate' && (
                <div className="flex flex-wrap gap-4 p-4 bg-indigo-50 rounded-2xl">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-indigo-700 uppercase">Aspect Ratio</label>
                    <select 
                      value={aspectRatio} 
                      onChange={(e) => setAspectRatio(e.target.value as AspectRatio)}
                      className="bg-white border border-indigo-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500"
                    >
              ...
              {['edit', 'video', 'analyze'].includes(activeTool) && (
                <div 
                  ...
                  className="border-2 border-dashed border-gray-200 rounded-3xl p-8 text-center hover:border-blue-400 cursor-pointer transition-all bg-gray-50/50"
                >
                  <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" accept="image/*" />
                  {selectedFile ? (
                    <div className="relative inline-block group">
                      <img src={selectedFile} alt="Preview" className="max-h-48 rounded-xl shadow-md" />
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center rounded-xl text-white transition-opacity">
                        Change Image
                      </div>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-xs text-gray-400 mt-2">Support: PNG, JPG (Max 5MB)</p>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="relative">
                <textarea
                  ...
                  className="w-full h-32 bg-gray-50 border border-gray-200 rounded-2xl p-6 focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all outline-none resize-none"
                />
              </div>
              ...
              <button
                ...
                disabled={loading}
                className={`w-full py-4 rounded-2xl text-white font-bold flex items-center justify-center space-x-2 transition-all ${
                  loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 shadow-xl hover:shadow-2xl hover:-translate-y-1'
                }`}
              >
                {loading ? (
                  <>
          ...
          {(result || loading) && (
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden animate-fade-up">
              <div className="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">AI Result</span>
                {result && !result.startsWith('data:') && !result.startsWith('blob:') && (
                  <button onClick={() => playTTS(result)} className="text-blue-600 hover:text-blue-700 flex items-center space-x-1">
              ...
              <div className="p-8">
                {loading ? (
                  <div className="flex flex-col items-center justify-center py-12 space-y-4">
                    ...
                    <p className="text-gray-500 font-medium">Generating magic...</p>
                  </div>
                ) : (
                  <>
                    {result?.startsWith('data:image') ? (
                      <div className="flex flex-col items-center">
                        <img src={result} alt="AI Generated" className="max-w-full rounded-2xl shadow-lg" />
                        <a href={result} download="nis-ai-image.png" className="mt-6 text-blue-600 font-bold underline">Download High Res</a>
                      </div>
                    ) : result?.startsWith('blob:') ? (
                      <div className="flex flex-col items-center">
                        <video src={result} controls className="max-w-full rounded-2xl shadow-lg" />
                        <p className="mt-4 text-xs text-gray-400">Video generated with Veo 3.1 Fast</p>
                      </div>
                    ) : (
                      <div className="prose prose-blue max-w-none whitespace-pre-wrap text-gray-700 leading-relaxed font-medium">
                        {result}
                      </div>
                    )}
                  </>
